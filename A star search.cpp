...
int l; // indexer to check wether values arrays values match new minimum , USED TO CHECK LOOP IN THE GRAPH.
	...
	do{
		...
		while (r < 20)
			...
			else
			{
				...
				// temp will be minimum for a nodes corresponding adjaent nodes ( nodes f+ g value)
				// temp is assigned to minimum after i iteration( adjacent nodes number is i )
				
				
				int x, y;
				//m.erase(p->first);

				if (traverse[t - 1] == p->second)
				{
					x = p->first;
					p++;
					y = p->first;
				...
				break; // this break will execute only if else statement of while loop executes.
