...
void Queue::enqueue(int data) {
	node *temp = new node();	// new int (4) means initialize to 4 but here ( ) means initialize to NULL	// why the  ()
	temp->info = data;
	temp->next = NULL;
	if (front == NULL){
		front = temp;
	...
	rear = temp;		// rear = rear + 1 / rear =temp
}
...
public:
	Graph(int size = 2);			// this value will be active if "Graph ob" is called , meaning if nothing initialized then size = 2 will
										// be assigned. 
	~Graph();
...
Graph::Graph(int size) {
	...
	else n = size;				// if Graph ob (4) then n = 4 will be the case.

	A = new int*[n];	// see Jumping into C++ Alex Allain page 154
...
Graph::~Graph() {					// see Jumping into C++ Alex Allain page 154
	for (int i = 0; i < n; ++i)
		delete[] A[i];
	delete[] A;
}
...
//	A[u - 1][v - 1] = A[v - 1][u - 1] = 1;		// see through a 2D square matrix, that i write from the addEdge
	//function , if you make only A[u-1][v-1]=1 then of the vertices will be missing.
	A[u - 1][v - 1] = 1;
...
void Graph::BFS(int s) {
	...
	bool *explored = new bool[n + 1];			// allocate an array of size n+1 , where n is the number of vertices

	/** Initailized all vertices as unexplored */
	for (int i = 1; i <= n; ++i)
		explored[i] = false;			// explored[0]=explored[13]= true and all others in between are false .

	/** Push initial vertex to the queue */
	Q.enqueue(s);			// s is the root of the Grpah or initial vertex
